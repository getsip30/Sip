      ...
      {open && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          ...
          <motion.div initial={{ scale: 0.9, opacity: 0, y: 10 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0 }}
            ...
            {title && <div style={{ fontSize: 17, fontWeight: 700, color: TEXT, marginBottom: 8, fontFamily: "var(--font-space-mono), 'Space Mono', monospace" }}>{title}</div>}
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={onCancel}
                style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', color: MUTED, padding: '9px 18px', borderRadius: 20, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
                {cancelLabel}
              </motion.button>
              <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={onConfirm}
                style={{ background: danger ? 'rgba(220,38,38,0.15)' : 'rgba(112,181,249,0.15)', border: `1px solid ${danger ? 'rgba(220,38,38,0.4)' : 'rgba(112,181,249,0.4)'}`, color: danger ? '#F87171' : LINK, padding: '9px 18px', borderRadius: 20, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
                {confirmLabel}
              </motion.button>
            </div>
          </motion.div>

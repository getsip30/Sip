...
export async function POST(req: Request) {
  try {
    ...
    if (!cleanQuestion) return NextResponse.json({ error: 'Question is required and must be under 500 characters' }, { status: 400 });

    const mentorResult = await db.select().from(mentors).where(eq(mentors.id, mentorId));
    const mentor = mentorResult[0];
    if (!mentor) return NextResponse.json({ error: 'Mentor not found' }, { status: 404 });
    if (mentor.banned) return NextResponse.json({ error: 'This mentor is not accepting questions.' }, { status: 403 });
    ...
    // Moderate BEFORE persisting. The old flow inserted first and deleted after,
    // which left abusive content readable by the mentor in the meantime, and
    ...
    if (moderation.unavailable) {
      return NextResponse.json({ error: "We couldn't check that question right now. Please try again in a moment." }, { status: 503 });
    }

    const created = await db.insert(asks).values({
      mentorId, seekerClerkId: userId, seekerName, seekerEmail, question: cleanQuestion, seekerConsentToShow: !!consentToShow,
    }).returning();
      ...
      await transporter.sendMail({
        ...
        html: `
          <div style="font-family:sans-serif;max-width:520px;margin:0 auto;background:#0D1117;color:#E6EDF3;padding:40px;border-radius:16px;">
            ...
            <p style="color:#8B949E;font-size:14px;line-height:1.7;margin-bottom:24px;">"${escapeHtml(cleanQuestion)}"</p>
            ${flagWarning}
    ...
    return handleApiError(err, 'POST /api/asks');
  }
}
...
export async function GET(req: Request) {
  try {
    ...
    return handleApiError(err, 'GET /api/asks');
  }
}
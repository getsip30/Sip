      ...
      <section style={{ position: 'relative', minHeight: '60vh', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', justifyContent: 'center', padding: '100px 20px 40px', maxWidth: 1100, margin: '0 auto', textAlign: 'left', overflow: 'hidden' }}>
        ...
        <motion.div
          ...
          {user ? (
            ...
            <>
              <motion.button
                whileHover={{ scale: 1.03, boxShadow: `0 0 24px ${ACCENT}66` }}
                whileTap={{ scale: 0.97 }}
                onClick={() => document.getElementById('mentors')?.scrollIntoView({ behavior: 'smooth' })}
                style={{ display: 'flex', alignItems: 'center', gap: 8, background: ACCENT, color: '#fff', border: 'none', padding: '13px 24px', borderRadius: 8, fontSize: 15, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', transition: 'box-shadow 0.3s' }}>
                ...
                {sent ? (
                  ...
                  <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                    ...
                    <motion.button
                      ...
                      style={{ width: '100%', background: submitting ? 'rgba(59,130,246,0.4)' : ACCENT, color: '#fff', border: 'none', padding: '13px 0', borderRadius: 20, fontSize: 15, fontWeight: 600, cursor: submitting ? 'not-allowed' : 'pointer', fontFamily: 'inherit', transition: 'background 0.2s' }}>

...
export default function Dashboard() {
  ...
  // Clerk leaves firstName empty for email-code signups and OAuth providers that
  // don't hand back a given name, so prefer the name captured on the mentor
  // profile and only fall back to Clerk.
  const greetingName = mentor?.firstName || user?.firstName || user?.fullName || '';
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 10 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 10 }}
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <a href="https://www.linkedin.com/feed/?shareActive=true" target="_blank" rel="noopener noreferrer"
                  ...
                  style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: ACCENT, color: 'white', padding: '12px 0', borderRadius: 20, fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>                  open LinkedIn
                </a>
              </div>
      ...
      <motion.nav initial={{ y: -60, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.4 }}
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'nowrap', overflowX: 'auto', maxWidth: '70vw', scrollbarWidth: 'none' }}>
          ...
          {rolesLoaded && (isSeeker
            ? <RoleSwitchLink to="/seekers" role="seeker" label="switch to seeker" style={{ color: LINK, textDecoration: 'none', fontSize: 13, border: '1px solid rgba(112,181,249,0.2)', padding: '6px 14px', borderRadius: 20, flexShrink: 0, whiteSpace: 'nowrap' }} />
            : <Link href="/seekers/onboarding" className="desktop-only" style={{ color: MUTED, textDecoration: 'none', fontSize: 13, flexShrink: 0, whiteSpace: 'nowrap', border: '1px solid rgba(255,255,255,0.1)', padding: '6px 14px', borderRadius: 20 }}>become a seeker too</Link>)}
            {greetingName && <span className="desktop-only" style={{ color: MUTED, fontSize: 13, flexShrink: 0, whiteSpace: 'nowrap' }}>hey, {greetingName}</span>}
          <AccountMenu />
        </div>
      </motion.nav>
                      ...
                      <div key={n.id} style={{ background: SURFACE, border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: '20px 24px' }}>
                        ...
                        <div style={{ display: 'flex', gap: 8 }}>
                          ...
                          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }}
                            ...
                            style={{ background: 'rgba(220,38,38,0.1)', border: '1px solid rgba(220,38,38,0.2)', color: DANGER, padding: '7px 14px', borderRadius: 20, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
                            reject
                          </motion.button>
                        </div>
                      </div>
                      ...
                      <div key={n.id} style={{ background: SURFACE, border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: '20px 24px', opacity: n.featured === false ? 0.6 : 1 }}>
                        ...
                        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                          ...
                          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }}
                            ...
                            style={{ background: 'rgba(112,181,249,0.12)', border: '1px solid rgba(112,181,249,0.3)', color: LINK, padding: '7px 14px', borderRadius: 20, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
                            share
                          </motion.button>
                          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }}
                            onClick={() => deleteNote(n.id)} disabled={deletingNote === n.id}
                            style={{ background: 'rgba(220,38,38,0.1)', border: '1px solid rgba(220,38,38,0.2)', color: DANGER, padding: '7px 14px', borderRadius: 20, fontSize: 13, fontWeight: 600, cursor: deletingNote === n.id ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}>
                            ...
                            {!r.mentorFeedbackGiven ? (
                              <div style={{ background: BG, border: `1px solid ${BORDER}`, borderRadius: 12, padding: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
                                ...
                                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }}
                                  ...
                                  style={{ alignSelf: 'flex-end', background: 'rgba(112,181,249,0.12)', border: '1px solid rgba(112,181,249,0.3)', color: LINK, padding: '6px 14px', borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: (submittingFeedback === r.id || !feedbackRatings[r.id]) ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}>

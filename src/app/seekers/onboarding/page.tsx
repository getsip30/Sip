        ...
        <div style={{ maxWidth: 860, margin: '0 auto', padding: '90px 16px 60px' }}>
          ...
          {matches.map(m => (
            <div key={m.id} style={{ border: `1px solid ${BORDER}`, borderRadius: 12, padding: 20, marginBottom: 14 }}>
              ...
              <Link href={`/seekers?mentor=${m.id}`} style={{ display: 'inline-block', background: ACCENT, color: 'white', padding: '10px 20px', borderRadius: 20, fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>book a sip</Link>
            </div>
          ))}
